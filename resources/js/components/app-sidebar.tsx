...
import { BookOpen, ChevronDown, Flower, Folder, Layers, LayoutGrid, Stethoscope, Users, Volleyball } from 'lucide-react';
            ...
            {items.map((item) => (
                <SidebarMenuItem key={item.title}>
                    {item.children ? (
                                    ...
                                    {item.children.map((child) => (
                                        <Link
                                            ...
                                            className={clsx(
                                                ...
                                                'hover:bg-accent hover:text-accent-foreground',
                                                isChildActive(child) ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground',
                                            )}
                                            prefetch
                                        >
                        ...
                        <SidebarMenuButton asChild isActive={isItemActive(item)} tooltip={isCollapsed ? item.title : undefined}>
...
export function AppSidebar() {
    ...
    const handleMouseLeave = () => {
        if (isMobile || !autoExpanded.current) return;
    ...
    return (
        <Sidebar collapsible="icon" variant="inset" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            <SidebarHeader>
                <SidebarMenu>
                    <SidebarMenuItem>
