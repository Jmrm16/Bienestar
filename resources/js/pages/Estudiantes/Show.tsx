...
export default function Show({ estudiante, return_period_id, grupos = [], notas = [] }: Props) {
  ...
  const tutorsCount = new Set(
    grupos.flatMap((grupo) => grupo.tutores.map((tutor) => tutor.id)),
  ).size;

  const asignaturas = Array.from(
    new Map(
      grupos
        .filter((grupo) => grupo.asignatura?.id)
        .map((grupo) => [grupo.asignatura!.id, grupo.asignatura!.nombre]),
    ).values(),
  );
      ...
      <div className="flex flex-col gap-6 rounded-xl p-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              <span className="font-medium text-foreground">
                {estudiante.period ? `${estudiante.period.code}${estudiante.period.name ? ` · ${estudiante.period.name}` : ""}` : `#${estudiante.period_id}`}
              </span>
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <MetricCard
            ...
            icon={Users}
            color="cyan"
            detail="Relacionados por asistencias"
          />
          <MetricCard
          ...
          <MetricCard
            ...
            icon={BookOpen}
            color="green"
            detail="Registros del mismo periodo"
          />
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              La relacion con grupos se obtiene desde asistencias del mismo periodo. Si no hay asistencias cargadas,
              el estudiante puede aparecer sin grupo.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Programa academico</p>
            ...
            <div>
              <p className="text-sm text-muted-foreground">Grupo priorizado</p>
            ...
            <div>
              <p className="text-sm text-muted-foreground">Asignaturas detectadas</p>
                ...
                <div key={grupo.id} className="rounded-xl border p-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Docente: {textOrDash(grupo.docente)}
                      </p>
                    </div>

                    <div className="text-sm text-muted-foreground">
                      <p>Asistencias: <span className="font-medium text-foreground">{grupo.total_asistencias}</span></p>
                      <p>Primera fecha: <span className="font-medium text-foreground">{formatDate(grupo.primera_fecha)}</span></p>
                      <p>Ultima fecha: <span className="font-medium text-foreground">{formatDate(grupo.ultima_fecha)}</span></p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    {grupo.tutores.length === 0 ? (
                      <p className="text-sm text-muted-foreground">No hay tutores asignados a este grupo en el periodo.</p>
                    ) : (
                      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                        {grupo.tutores.map((tutor) => (
                          <div key={tutor.id} className="rounded-lg bg-muted/40 p-3">
                            ...
                            <p className="mt-1 text-sm text-muted-foreground">
                              Correo: {textOrDash(tutor.correo)}
                            </p>
                            <p className="text-sm text-muted-foreground">
                              Telefono: {textOrDash(tutor.telefono)}
                            </p>
                          </div>
                        ))}
                      </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Registros encontrados por identificacion y periodo.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {notas.length === 0 ? (
