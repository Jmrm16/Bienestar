...
export default function EstudiantesReportPanel({ rows, periodLabel }: { rows: EstudianteRow[]; periodLabel: string }) {
    const report = useMemo(() => {
        ...
        const serviceMap = new Map<string, { registros: number; estudiantes: Set<string>; actividades: Set<string> }>();
        ...
        <div className="space-y-6">
            <div className="space-y-2">
                ...
                <p className="text-muted-foreground text-sm">
                    Resumen del período <span className="text-foreground font-medium">{periodLabel}</span> con distribución por servicios, programas,
                    trimestres y grupos priorizados.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4">
                ...
                />
                <MetricCard title="Registros del período" value={report.totalRegistros} icon={Layers3} color="purple" detail="Filas importadas" />
                <MetricCard title="Servicios" value={report.totalServicios} icon={BarChart3} color="blue" detail="Servicios detectados" />
                <MetricCard title="Programas" value={report.totalProgramas} icon={PieChartIcon} color="green" detail="Programas académicos" />
            </div>

            <Card>
                <CardHeader>
                    <CardTitle>Informe rápido</CardTitle>
                    <CardDescription>Lectura ejecutiva del comportamiento del módulo en el período seleccionado.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <p className="text-muted-foreground text-sm leading-6">
                        Se encontraron <span className="text-foreground font-semibold">{formatInt(report.estudiantesUnicos)}</span> estudiantes únicos
                        distribuidos en <span className="text-foreground font-semibold">{formatInt(report.totalServicios)}</span> servicios y{' '}
                        <span className="text-foreground font-semibold">{formatInt(report.totalProgramas)}</span> programas.
                        {report.topService ? (
                            <>
                                {' '}
                                El servicio con mayor cobertura es <span className="text-foreground font-semibold">
                                    {report.topService.label}
                                </span>{' '}
                                con <span className="text-foreground font-semibold">{formatInt(report.topService.estudiantes)}</span> estudiantes.
                            </>
                        ) : null}
                        {report.topProgram ? (
                            <>
                                ...
                                <span className="text-foreground font-semibold">{report.topProgram.label}</span>.
                            </>
                        ) : null}
                        {report.topTrimester ? (
                            <>
                                {' '}
                                El trimestre más representado es <span className="text-foreground font-semibold">{report.topTrimester.label}</span>.
                            </>
                        ) : null}
                    </p>

                    <div className="flex flex-wrap gap-2">
                        {report.topService ? <Badge variant="secondary">Servicio líder: {report.topService.label}</Badge> : null}
                        {report.topProgram ? <Badge variant="secondary">Programa líder: {report.topProgram.label}</Badge> : null}
                        {report.topTrimester ? <Badge variant="secondary">Trimestre líder: {report.topTrimester.label}</Badge> : null}
                    </div>
                </CardContent>
            </Card>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Informe consolidado de estudiantes, registros y actividades detectadas.</CardDescription>
                </CardHeader>
                <CardContent>
