...
import { Delete, Eye, PencilLine, Search, X } from 'lucide-react';
        ...
        <div className="bg-card text-card-foreground flex flex-col gap-4 rounded-xl border p-4 shadow-sm">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div className="grid w-full gap-2 md:grid-cols-[minmax(0,1.4fr)_220px_220px_auto_auto] lg:max-w-[980px]">
                    ...
                    <Select value={filters.servicio || 'all'} onValueChange={(value) => applyFilters({ servicio: value === 'all' ? '' : value })}>
                        <SelectTrigger>
                    ...
                    <Select value={filters.trimestre || 'all'} onValueChange={(value) => applyFilters({ trimestre: value === 'all' ? '' : value })}>
                        <SelectTrigger>
                    ...
                    <Button variant="outline" onClick={() => applyFilters({ q: searchDraft.trim() })}>
                        Buscar
                    </Button>

                    <Button variant="ghost" onClick={clearFilters} disabled={!hasActiveFilters && searchDraft === ''}>
            ...
            <div className="text-muted-foreground flex flex-col gap-2 text-sm sm:flex-row sm:items-center sm:justify-between">
                ...
                {hasActiveFilters ? (
                    <span>
                        Filtros activos:{' '}
                        {[filters.q && 'búsqueda', filters.servicio && 'servicio', filters.trimestre && 'trimestre'].filter(Boolean).join(' · ')}
                    </span>
                ) : null}
            </div>
                                ...
                                <TableRow key={r.id}>
                                    <TableCell className="align-top">
                                        <div className="space-y-2">
                                            <div>
                                                <p className="font-medium">{`${r.nombres ?? ''} ${r.apellidos ?? ''}`.trim() || '—'}</p>
                                                <p className="text-muted-foreground text-sm">{textOrDash(r.identificacion)}</p>
                                            </div>

                                            <div className="flex flex-wrap gap-2">
                                                {r.sexo ? <Badge variant="outline">{r.sexo}</Badge> : null}
                                                {r.estamento ? <Badge variant="secondary">{r.estamento}</Badge> : null}
                                            </div>
                                        </div>
                                    </TableCell>
                                    ...
                                    <TableCell className="align-top">
                                        <div className="max-w-[260px] space-y-1">
                                            ...
                                            <p className="text-muted-foreground text-sm" title={r.dependencia ?? ''}>
                                    ...
                                    <TableCell className="align-top">
                                        <div className="max-w-[360px] space-y-2">
                                            <div className="flex flex-wrap gap-2">
                                                <Badge variant="secondary">{textOrDash(r.servicio, 'Sin servicio')}</Badge>
                                                <Badge variant="outline">{textOrDash(r.trimestre, 'Sin trimestre')}</Badge>
                                            </div>

                                            <div>
                                                <p className="text-sm font-medium">Actividad</p>
                                                <p className="text-muted-foreground line-clamp-2 text-sm" title={r.actividad ?? ''}>
                                                    {textOrDash(r.actividad, 'Sin actividad')}
                                                </p>
                                            </div>

                                            <p className="text-muted-foreground text-xs">Responsable: {textOrDash(r.responsable, 'No registrado')}</p>
                                        </div>
                                    </TableCell>
                                    ...
                                    <TableCell className="space-x-2 text-right">
                                        ...
                                        <Button variant="ghost" className="text-destructive" onClick={() => openDelete(r)} title="Eliminar">
            ...
            {(rows.prev_page_url || rows.next_page_url) && (
                <div className="flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-muted-foreground text-sm">Navegación por páginas para mantener el listado liviano.</p>
                    <div className="flex gap-2">
                        <Button variant="outline" disabled={!rows.prev_page_url} onClick={() => applyFilters({}, Math.max(1, rows.current_page - 1))}>
                            Anterior
                        </Button>
                        <Button variant="outline" disabled={!rows.next_page_url} onClick={() => applyFilters({}, rows.current_page + 1)}>
                            Siguiente
                        </Button>
                    </div>
                </div>
            ...
            >
                <DialogContent className="sm:max-w-[720px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>Actualiza la información del estudiante seleccionado.</DialogDescription>
                    </DialogHeader>
                    ...
                    {selected && (
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div className="space-y-2">
                                <Label>Sexo</Label>
                                <Input value={selected.sexo ?? ''} onChange={(e) => setSelected({ ...selected, sexo: e.target.value })} />
                            </div>

                            <div className="space-y-2">
                                <Label>Nombres</Label>
                                <Input value={selected.nombres ?? ''} onChange={(e) => setSelected({ ...selected, nombres: e.target.value })} />
                            </div>

                            <div className="space-y-2">
                                <Label>Apellidos</Label>
                                <Input value={selected.apellidos ?? ''} onChange={(e) => setSelected({ ...selected, apellidos: e.target.value })} />
                            </div>
                            ...
                            <div className="space-y-2 md:col-span-2">
                                ...
                                <Input value={selected.servicio ?? ''} onChange={(e) => setSelected({ ...selected, servicio: e.target.value })} />
                            </div>

                            <div className="space-y-2 md:col-span-2">
                                <Label>Actividad</Label>
                                <Input value={selected.actividad ?? ''} onChange={(e) => setSelected({ ...selected, actividad: e.target.value })} />
                            </div>

                            <div className="space-y-2">
                                <Label>Trimestre</Label>
                                <Input value={selected.trimestre ?? ''} onChange={(e) => setSelected({ ...selected, trimestre: e.target.value })} />
                            </div>
