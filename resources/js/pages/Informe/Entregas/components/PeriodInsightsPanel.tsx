...
const stickyHeaderCellClassName = 'sticky left-0 z-10 w-[220px] bg-background/95 px-2 text-[11px] backdrop-blur sm:w-[320px] sm:text-sm';
const stickyBodyCellClassName = 'sticky left-0 z-10 bg-background/95 backdrop-blur';

function sumMetric(perWindow: PerWindow, windows: WindowInsightRow[], metric: 'estudiantes' | 'asistencias') {
    let total = 0;
...
function buildInsightsSummary(slice: InsightSlice | undefined) {
    ...
    const hasUniqueTotals = carreras.some(
        (carrera) => typeof carrera.unique_estudiantes_total === 'number' || typeof carrera.unique_asistencias_total === 'number',
    );

    const totalEstudiantes = hasUniqueTotals
        ? carreras.reduce((acc, carrera) => acc + (carrera.unique_estudiantes_total ?? 0), 0)
        : carreras.reduce((acc, carrera) => acc + sumMetric(carrera.per_window, windows, 'estudiantes'), 0);

    const totalAsistencias = hasUniqueTotals
        ? carreras.reduce((acc, carrera) => acc + (carrera.unique_asistencias_total ?? 0), 0)
        : carreras.reduce((acc, carrera) => acc + sumMetric(carrera.per_window, windows, 'asistencias'), 0);
...
function formatCompactMetricHeader(window: WindowInsightRow, windows: WindowInsightRow[], metric: 'estudiantes' | 'asistencias', index: number) {
        ...
        <>
            {windows.map((w, index) => (
                <React.Fragment key={w.window_id}>
                    <TableHead className="px-2 text-right text-[11px] whitespace-nowrap sm:text-sm">
            ...
            <TableHead className="px-2 text-right text-[11px] font-bold whitespace-nowrap sm:text-sm">
...
function DataCells({
    ...
    const totalEst = typeof uniqueTotal === 'number' ? uniqueTotal : sumMetric(perWindow, windows, 'estudiantes');
    const totalAsis = typeof uniqueAsisTotal === 'number' ? uniqueAsisTotal : sumMetric(perWindow, windows, 'asistencias');
    ...
    return (
        <>
            {windows.map((w) => {
                ...
                return (
                    <React.Fragment key={w.window_id}>
                        <TableCell className={cn('px-2 text-right text-xs whitespace-nowrap sm:text-sm', !cell && 'text-muted-foreground')}>
            ...
            <TableCell className="px-2 text-right text-xs font-bold whitespace-nowrap sm:text-sm">{totalEst}</TableCell>
            <TableCell className="px-2 text-right text-xs font-bold whitespace-nowrap sm:text-sm">{totalAsis}</TableCell>
        </>
    );
}
...
function InsightsTableSection({
    ...
    const totalEstudiantes = typeof totalEstudiantesOverride === 'number' ? totalEstudiantesOverride : summary.totalEstudiantes;
    ...
    const toggleCarrera = (id: number) => setOpenCarreras((s) => ({ ...s, [id]: !s[id] }));
    ...
    return (
        <Card className="gap-0 overflow-hidden">
            <CardHeader className="gap-1 px-4 sm:px-6">
                ...
                <CardDescription>{contextLabel ? contextLabel : 'Resumen por carrera, asignatura y tutor.'}</CardDescription>
            </CardHeader>
            ...
            <CardContent className="p-0">
                {windows.length === 0 ? (
                    <div className="text-muted-foreground flex min-h-40 items-center justify-center px-4 py-8 text-center">
                    ...
                    <ScrollArea className="w-full">
                        {showSummary ? (
                            <div className="grid gap-3 px-4 pt-4 pb-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
                                <div className="bg-muted/15 rounded-2xl border p-4">
                                    <p className="text-muted-foreground text-[11px] font-medium tracking-[0.18em] uppercase">Total de la selección</p>
                                    <div className="mt-3 grid gap-3 sm:grid-cols-2">
                                        <div className="bg-background rounded-xl border px-4 py-3">
                                            <p className="text-muted-foreground text-xs">Estudiantes únicos</p>
                                            <p className="mt-1 text-2xl font-semibold">{totalEstudiantes.toLocaleString('es-CO')}</p>
                                        </div>
                                        <div className="bg-background rounded-xl border px-4 py-3">
                                            <p className="text-muted-foreground text-xs">Asistencias únicas</p>
                                            <p className="mt-1 text-2xl font-semibold">{summary.totalAsistencias.toLocaleString('es-CO')}</p>
                                        </div>
                                    </div>
                                </div>

                                <div className="bg-background rounded-2xl border p-4">
                                    <p className="text-muted-foreground text-[11px] font-medium tracking-[0.18em] uppercase">Resumen por entrega</p>
                                    <div className="mt-3 grid gap-2">
                                        {summary.perWindowTotals.map((window) => (
                                            <div
                                                key={window.window_id}
                                                className="bg-muted/10 flex items-center justify-between gap-3 rounded-xl border px-3 py-2"
                                            >
                        ...
                        <div className="text-muted-foreground px-4 pt-4 pb-2 text-xs sm:hidden">Desliza horizontalmente para ver toda la tabla</div>
                        <div style={{ minWidth: `${tableMinWidth}px` }}>
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead className={stickyHeaderCellClassName}>Nombre</TableHead>
                                                ...
                                                <TableRow className="bg-muted/5 font-medium">
                                                    <TableCell className={cn(stickyBodyCellClassName, 'px-2')}>
                                                        <button
                                                            ...
                                                            className="hover:text-foreground inline-flex w-full min-w-0 items-center gap-2 py-1 text-left text-xs transition-colors sm:text-sm"
                                                        >
                                                            {isOpenC ? (
                                                        ...
                                                        return (
                                                            <React.Fragment key={a.id}>
                                                                <TableRow className="text-sm">
                                                                    <TableCell className={cn(stickyBodyCellClassName, 'px-2 pl-8')}>
                                                                        <button
                                                                            type="button"
                                                                ...
                                                                {isOpenA &&
                                                                    (a.tutores ?? []).map((t) => (
                                                                        <TableRow key={t.id} className="text-muted-foreground text-sm">
                                                                            <TableCell
                                                                                className={cn(
...
function ReportTreeTable({
    ...
    const sectionTitle = useMemo(() => title ?? 'Detalle por carrera, asignatura y tutor', [title]);

    return (
        <div className="space-y-4 [contain-intrinsic-size:980px] [content-visibility:auto]">
            <div>
    ...
    return (
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
            ...
            <div className="bg-background rounded-2xl border p-4">
                ...
                <div className="mt-3 grid gap-2">
                    {summary.perWindowTotals.map((window) => (
                        <div key={window.window_id} className="bg-muted/10 flex items-center justify-between gap-3 rounded-xl border px-3 py-2">
                            <div className="min-w-0">
                                <p className="truncate text-sm font-medium">{formatWindowHeaderLabel(window) || `Informe ${window.window_id}`}</p>
                                <p className="text-muted-foreground text-xs">
                                    {window.estudiantes.toLocaleString('es-CO')} estudiantes · {window.asistencias.toLocaleString('es-CO')}{' '}
                                    asistencias
                                </p>
                            </div>
                        </div>
