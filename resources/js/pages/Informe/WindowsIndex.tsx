...
const categoryOrder: WindowCategory[] = [
  "corte_1",
  "corte_2",
  "corte_3",
  "habilitacion",
...
function formatWindowOptionLabel(window: Window) {
  return `${window.name} · ${window.tutor_type}`;
}
...
export default function WindowsIndex({
  ...
  const [expandedCategories, setExpandedCategories] = useState<
    Partial<Record<WindowCategory, boolean>>
  >({});
  ...
  const defaultId =
    ...
    Number(sortedWindows?.[sortedWindows.length - 1]?.id) ||
    Number(sortedWindows?.[0]?.id) ||
    0;

  const defaultCategory = useMemo(() => {
    const defaultWindow =
      sortedWindows.find((window) => Number(window.id) === defaultId) ?? sortedWindows[0] ?? null;
  ...
  const [selectedCategoryFilter, setSelectedCategoryFilter] = useState<WindowCategory | "">(
    defaultCategory ?? ""
  );
  const [selectedResolutionFilter, setSelectedResolutionFilter] =
    useState<WindowTutorSelection>("ALL");
    ...
    const selectedIds = sortedWindows
      .filter((window) => {
        ...
        const matchesResolution =
          selectedResolutionFilter === "ALL" || window.tutor_type === selectedResolutionFilter;
  ...
  const selectedWindowNames = useMemo(
    () => Array.from(new Set(selectedWindows.map((window) => window.name))).join(" · "),
    [selectedWindows]
  );

  const selectedCategoryMeta = selectedCategoryFilter
    ? windowCategoryCopy[selectedCategoryFilter]
    : null;
  ...
  const handleExcelExport = async () => {
    ...
    try {
      ...
      const resolvedName = filenameMatch?.[1]
  ...
  const setWindowCategory = (category: WindowCategory) => {
    setForm((current) => {
      ...
      const shouldReplaceName =
        !current.name.trim() || current.name === previousPreset.suggestedName;
      const shouldReplaceItems =
        current.required_items.length === 0 ||
        JSON.stringify(current.required_items) === JSON.stringify(previousPreset.defaultItems);
  ...
  const clearWindowData = (w: Window) => {
    if (
      !window.confirm(
        `Se vaciarán los datos cargados de "${w.name}" (${w.tutor_type}) y la entrega seguirá existiendo. ¿Deseas continuar?`
      )
    ) {
      return;
    }
    ...
    <AppLayout breadcrumbs={breadcrumbs(period)}>
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 p-4 sm:p-6">
          ...
          <CardHeader className="gap-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
                <BulkTutorAttendanceImportDialog
                  periodId={period.id}
                  windows={sortedWindows}
                  onCompleted={refreshReportsView}
                />
                    ...
                    <DialogHeader>
                      <div className="border-b px-6 py-5">
                        ...
                        <DialogDescription className="mt-2">
                          Define el corte, la resolución, las fechas y lo que deben subir los
                          tutores en esta entrega del periodo.
                        </DialogDescription>
                      </div>
                    </DialogHeader>
                      ...
                      <div className="grid gap-5">
                        <div className="grid gap-4 md:grid-cols-2">
                          ...
                          <Select
                            ...
                            disabled={savingWindow}
                            onValueChange={(v: WindowTutorSelection) =>
                              setForm((f) => ({ ...f, tutor_type: v }))
                            }
                          >
                            <SelectTrigger>
                        ...
                        <div className="rounded-2xl border bg-card p-4">
                          <div className="grid gap-4 md:grid-cols-2">
                            <div className="space-y-2">
                              ...
                              <div className="flex min-h-11 items-center gap-3 rounded-xl border bg-muted/20 px-3">
                                <Switch
                                  checked={form.is_published}
                                  disabled={savingWindow}
                                  onCheckedChange={(v) =>
                                    setForm((f) => ({ ...f, is_published: Boolean(v) }))
                                  }
                                />
                        ...
                        <div className="space-y-3 rounded-2xl border bg-card p-4">
                          ...
                          <div className="grid gap-3 md:grid-cols-2">
                            {requiredItemOptions.map((item) => (
                              <label
                                key={item.value}
                                className="flex min-h-[112px] items-start gap-3 rounded-xl border bg-muted/15 p-3 transition hover:border-border hover:bg-muted/30"
                              >
                                <Checkbox
                                  checked={form.required_items.includes(item.value)}
                                  disabled={savingWindow}
                                  onCheckedChange={(checked) =>
                                    toggleRequiredItem(item.value, Boolean(checked))
                                  }
                                />
        ...
        <div>
                                ...
                                <div className="min-w-0 space-y-1">
                                  <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <Badge variant="outline" className="font-normal">
                                      ...
                                      {group.windows.length === 1 ? "entrega" : "entregas"}
                                    </Badge>
                                    <span className="text-sm text-muted-foreground">
          ...
          <Card className="gap-0 [content-visibility:auto] [contain-intrinsic-size:760px]">
            <CardHeader className="gap-4">
              ...
              <div className="rounded-2xl border bg-muted/20 p-4">
                <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-end">
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-2">
                      <Label className="text-xs text-muted-foreground">Qué quieres ver</Label>
                      <Select
                        value={selectedResolutionFilter}
                        onValueChange={(value) =>
                          setSelectedResolutionFilter(value as WindowTutorSelection)
                        }
                      >
                        <SelectTrigger>
                        ...
                        <SelectContent>
                          {groupedWindows.map((group) => {
                            const names = Array.from(
                              new Set(group.windows.map((window) => window.name))
                            );
                            const description =
                              names.length === 1
                                ? `${group.meta.label} · ${names[0]}`
                                : group.meta.label;
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        window.open(
                          `${route("reports.period.export_pdf", period.id)}?autoprint=1`,
                          "_blank"
                        )
                      }
                    >
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant="outline" className="font-normal">
                    ...
                    {selectedWindows.length === 1 ? "entrega incluida" : "entregas incluidas"}
                  </Badge>
                </div>
              </div>
            </CardHeader>
          ...
          <CardContent className="space-y-4 pt-6">
            {chartsLoading && !chartCache[selectionCacheKey] ? (
              <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed bg-muted/20 p-8 text-center text-sm text-muted-foreground">
