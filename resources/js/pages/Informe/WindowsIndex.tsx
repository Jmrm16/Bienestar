...
export default function WindowsIndex({ period, windows, insights }: Props) {
  ...
  const submit = () => {
    ...
    router.post(
      ...
      { _method: editing ? "put" : undefined, ...payload },
      {
        ...
        onSuccess: () => {
          toast.success(
            editing ? "Entrega/corte actualizada" : "Entrega/corte creada"
          );
          setOpen(false);
          resetForm();
        },
  ...
  // Asignar entrega/corte a todos los tutores de un tipo (R1 / R2)
  const assignAll = (w: Window) => {
    router.post(
      ...
      {},
      {
        ...
        onSuccess: () =>
          toast.success(
            `Entrega/corte asignada a todos los tutores de tipo ${w.tutor_type}`
          ),
  ...
  // Ver entregas por tutor (ruta que implementas tú en backend)
  const viewSubmissions = (w: Window) => {
    router.visit(
      `/reportes/periodos/${period.id}/entregas/${w.id}/tutores`,
      { preserveScroll: true }
    );
  };
  ...
  const formatDate = (value?: string | null) => {
    if (!value) return "";
    // Si viene como "2025-12-09T10:30:00" o tipo ISO, recortamos
      ...
      <div className="flex flex-col gap-6 p-4">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Usa estas entregas para habilitar los cortes de asistencias que
              deben subir los tutores (Corte 1, Corte 2, Informe final, etc.).
            </p>
          </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="R1">
                        Tutores de Primera resolución (R1)
                      </SelectItem>
                      <SelectItem value="R2">
                        Tutores de Segunda resolución (R2)
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Especifica qué deben subir los tutores en este corte (ej: archivo de asistencias, acta, informe PDF, etc.)"
                  />
                </div>
              </div>
        ...
        >
          ...
          <div className="mt-3 text-sm text-muted-foreground flex items-center gap-2">
            ...
            {totalPublicadas} publicadas · {windows.length - totalPublicadas}{" "}
            ocultas
          </div>
        </motion.div>
        ...
        <div className="rounded-lg border p-4 text-sm text-muted-foreground">
          <div className="flex items-center gap-2 font-medium">
            ...
            Tips de uso para cortes de asistencias
          </div>
          <ul className="mt-2 list-disc list-inside space-y-1">
            <li>
              Crea tres entregas si quieres manejar <strong>Corte 1</strong>,{" "}
              <strong>Corte 2</strong> e <strong>Informe final</strong>.
            </li>
            <li>
              Usa el campo <em>Instrucciones</em> para explicar qué archivo de
              asistencias deben subir.
            </li>
            <li>
              <strong>Publicada</strong> controla si el corte aparece en el
              portal de tutores.
            </li>
            <li>
              <strong>Asignar a todos</strong> crea/actualiza registros
              pendientes para todos los tutores del tipo seleccionado (R1 / R2).
            </li>
            <li>
              El botón <strong>Ver entregas</strong> (cuando implementes la
              ruta) te permitirá ver qué tutores ya subieron archivos y quiénes
              siguen pendientes.
            </li>
          </ul>
        </div>
