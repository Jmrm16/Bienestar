...
import { AlertTriangle, CalendarClock, ClipboardList, Package2, Pill, Plus } from 'lucide-react';
        ...
        return (
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>El panel de inventario y actividades necesita las tablas nuevas del módulo.</CardDescription>
                </CardHeader>
                <CardContent>
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-5">
                <MetricCard title="Medicamentos" value={safeStats.total_medicamentos} icon={Pill} color="cyan" detail="Lotes registrados" />
                <MetricCard title="Stock bajo" value={safeStats.stock_bajo} icon={AlertTriangle} color="purple" detail="Requieren revisión" />
                <MetricCard
                ...
                />
                <MetricCard title="Entregas del mes" value={safeStats.entregas_mes} icon={Package2} color="green" detail="Salidas registradas" />
                <MetricCard
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between gap-4">
                            <div className="space-y-1">
                                ...
                                <CardDescription>Controla entradas, vencimientos, stock disponible y ubicación de cada lote.</CardDescription>
                            </div>
                                                ...
                                                <TableRow key={item.id}>
                                                    <TableCell>
                                                        <div className="space-y-1">
                                                            ...
                                                            <p className="text-muted-foreground text-xs">Ubicación: {textOrDash(item.ubicacion)}</p>
                                                        </div>
                                                    </TableCell>
                                                    ...
                                                    <TableCell>
                                                        <div className="space-y-1">
                                                            ...
                                                            <p className="text-muted-foreground text-xs">Proveedor: {textOrDash(item.proveedor)}</p>
                                                        </div>
                                                    </TableCell>
                                                    ...
                                                    <TableCell>
                                                        {item.paciente ? (
                                                            <div className="space-y-1">
                                                                ...
                                                                <p className="text-muted-foreground text-xs">{item.paciente.documento}</p>
                                                            </div>
                                                        ) : (
                                                            'Sin paciente'
                                                        )}
                ...
                <DialogContent className="sm:max-w-3xl">
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                value={inventoryForm.presentacion}
                                onChange={(e) => setInventoryForm((f) => ({ ...f, presentacion: e.target.value }))}
                            />
                        </div>
                        <div className="space-y-2">
                            <Label>Lote</Label>
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                type="date"
                                value={inventoryForm.fecha_entrada}
                                onChange={(e) => setInventoryForm((f) => ({ ...f, fecha_entrada: e.target.value }))}
                            />
                        </div>
                        <div className="space-y-2">
                            ...
                            <Input
                                type="date"
                                value={inventoryForm.fecha_vencimiento}
                                onChange={(e) => setInventoryForm((f) => ({ ...f, fecha_vencimiento: e.target.value }))}
                            />
                        </div>
                        <div className="space-y-2">
                            ...
                            <Input
                                type="number"
                                min="1"
                                value={inventoryForm.cantidad_inicial}
                                onChange={(e) => setInventoryForm((f) => ({ ...f, cantidad_inicial: e.target.value }))}
                            />
                        </div>
                        <div className="space-y-2">
                            <Label>Unidad</Label>
                        ...
                        <div className="space-y-2 md:col-span-2">
                            ...
                            <Textarea
                                value={inventoryForm.observaciones}
                                onChange={(e) => setInventoryForm((f) => ({ ...f, observaciones: e.target.value }))}
                            />
                        </div>
                    </div>
                    <DialogFooter>
            ...
            <Dialog open={deliveryOpen} onOpenChange={setDeliveryOpen}>
                <DialogContent className="sm:max-w-3xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>Registra la salida de medicamentos y actualiza el stock disponible.</DialogDescription>
                    </DialogHeader>
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                            <Label>Medicamento</Label>
                            <Select
                                value={deliveryForm.medicamento_id}
                                onValueChange={(value) => setDeliveryForm((f) => ({ ...f, medicamento_id: value }))}
                            >
                                <SelectTrigger>
                        ...
                        <div className="space-y-2">
                            ...
                            <Select
                                value={deliveryForm.paciente_id}
                                onValueChange={(value) => setDeliveryForm((f) => ({ ...f, paciente_id: value }))}
                            >
                                <SelectTrigger>
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                type="date"
                                value={deliveryForm.fecha_entrega}
                                onChange={(e) => setDeliveryForm((f) => ({ ...f, fecha_entrega: e.target.value }))}
                            />
                        </div>
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                onChange={(e) => setDeliveryForm((f) => ({ ...f, cantidad: e.target.value }))}
                            />
                        </div>
                        <div className="space-y-2">
                            <Label>Responsable</Label>
                            <Input
                                value={deliveryForm.responsable}
                                onChange={(e) => setDeliveryForm((f) => ({ ...f, responsable: e.target.value }))}
                            />
                        </div>
                        <div className="space-y-2">
                            <Label>Destino</Label>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleDeliveryCreate} disabled={!deliveryForm.medicamento_id}>
                            Guardar
                        </Button>
                    </DialogFooter>
                </DialogContent>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                onChange={(e) => setActivityForm((f) => ({ ...f, tipo: e.target.value }))}
                                placeholder="Ej. Curación, control, administración"
                            />
                        </div>
                        <div className="space-y-2">
                            <Label>Paciente</Label>
                            <Select
                                value={activityForm.paciente_id}
                                onValueChange={(value) => setActivityForm((f) => ({ ...f, paciente_id: value }))}
                            >
                                <SelectTrigger>
                                    <SelectValue placeholder="Paciente opcional" />
                                </SelectTrigger>
                        ...
                        <div className="space-y-2 md:col-span-2">
                            ...
                            <Textarea
                                ...
                                onChange={(e) => setActivityForm((f) => ({ ...f, descripcion: e.target.value }))}
                            />
                        </div>
                        <div className="space-y-2 md:col-span-2">
                            <Label>Observaciones</Label>
                            <Textarea
                                value={activityForm.observaciones}
                                onChange={(e) => setActivityForm((f) => ({ ...f, observaciones: e.target.value }))}
                            />
                        </div>
                    </div>
                    <DialogFooter>
                        ...
                        <Button onClick={handleActivityCreate} disabled={!activityForm.tipo || !activityForm.descripcion}>
                            Guardar
                        </Button>
                    </DialogFooter>
                </DialogContent>
