...
export default function TutorAsignaturasField({
  asignaturas,
  selectedIds,
  onToggle,
  onClear,
  ...
  const selectedSubjects = useMemo(
    () => asignaturas.filter((subject) => selectedIds.includes(subject.id)),
    [asignaturas, selectedIds]
  );
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            {hasCareer
              ? `Mostrando solo asignaturas de ${carreraNombre}.`
            ...
            filteredSubjects.map((subject) => (
              ...
              >
                ...
                <div className="min-w-0 text-sm">
                  <p className="font-medium leading-tight">{subject.nombre}</p>
                  {subject.codigo && (
